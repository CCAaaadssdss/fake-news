...
class Bert_Model(nn.Module):
    ...
    def __init__(self, args, max_len=200):
        ...
        self.predictor = nn.Linear(self.hidden_size, self.label_num)
        self.attn_score = nn.Linear(768, 1)
        self.avg = torch.randn(self.evd_size).cuda()

        # 添加 Gated Affine Transformation 中的线性变换参数
        self.W_alpha = nn.Linear(self.evd_size, self.hidden_size)
        self.W_beta = nn.Linear(2 * self.hidden_size, self.hidden_size)
        self.W_gamma = nn.Linear(2 * self.evd_size, self.hidden_size)
        self.credibility_transform = nn.Linear(3, 768)

    def gated_affine_transformation(self, source_credibility, interaction_semantics):
        """
        实现 Gated Affine Transformation
        """
        # 将 source_credibility 通过线性层变换为 [batch_size, 768]
        source_credibility = source_credibility.float()
        source_credibility = self.credibility_transform(source_credibility)

        alpha = self.W_alpha(source_credibility)
        beta = self.W_beta(interaction_semantics)
        gamma = self.W_gamma(interaction_semantics)

        gate = torch.sigmoid(beta)
        fused_representation = gate * alpha + gamma
        return fused_representation
    

    
    def forward(self, claim_input_id, claim_attn_mask, snippet_input_id, snippet_token_type_id, snippet_attn_mask, source_credibility=None, debias=0.0, evd_output=False):
        ...
        if source_credibility is not None:
            # 调用 Gated Affine Transformation
            claim_snippet_cls = self.gated_affine_transformation(source_credibility, claim_snippet_cls)
            # claim_snippet_cls = self.gated_affine_transformation(source_credibility, claim_snippet_cls)

        if evd_output is True:
            return snippet_cls

        debias_value = debias.item() if isinstance(debias, torch.Tensor) else debias
        if debias_value != 0.0:
...
class MAC(nn.Module):
    ...
    def forward(self, claim_id, claim_mask, evidence_id, _, evidence_mask, debias=0.0, evd_output=False):
        ...
        avg, word_att_weights = self._word_level_attention(
            left_tsr=claim.unsqueeze(1).expand(B, 10, -1).reshape(B * 10, -1), 
            right_tsr=bilstm_evidence, 
            right_mask=evidence_mask.view(B * 10, -1)
        )  # avg: (B*10, heads_for_words*hidden_size)
        ...
        if debias is not None and (debias != 0.0).any():
            if self.avg is None:
                raise Exception("Model.avg is unavailable")

            output2 = torch.cat((claim.detach().clone(), self.avg.unsqueeze(0).expand(B, -1)), dim=-1)

            logits = self.out(output)
            logits_debias = self.out(output2)

            min_dim = min(logits.shape[1], logits_debias.shape[1])
            logits = logits[:, :min_dim]
            logits_debias = logits_debias[:, :min_dim]

            # 数值稳定化
            logits = logits - logits.max(dim=-1, keepdim=True).values
            logits_debias = logits_debias - logits_debias.max(dim=-1, keepdim=True).values

            logits = torch.softmax(logits, dim=-1)
            logits_debias = torch.softmax(logits_debias, dim=-1)

            if debias.dim() == 1:
                debias = debias.view(-1, 1).expand_as(logits)
            elif debias.shape[0] != logits.shape[0]:
                debias = debias[:logits.shape[0], :min_dim].expand_as(logits)

            debias = torch.clamp(debias, min=-1e9, max=1e9)
            debias = torch.sum(debias, dim=1, keepdim=True).expand(-1, 2)

            return logits - debias * logits_debias
