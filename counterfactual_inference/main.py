...
class dataset(Dataset):
    ...
    def __init__(self, data: dict) -> None:
        ...
        self.snippets_input_id = torch.from_numpy(data['snippets_input_id'])
        ...
        # 添加来源可信度向量
        self.source_credibility = torch.from_numpy(data['source_credibility'])
    ...
    def __getitem__(self, index):
        return (self.claim_input_id[index], self.claim_mask[index],
                self.snippets_input_id[index], self.snippets_token_type_id[index], 
                self.snippets_mask[index], self.label[index], self.source_credibility[index])

def custom_collect_fn(batch):
    claims = [t[0] for t in batch]
    claim_masks = [t[1] for t in batch]
    snippets = [t[2] for t in batch]
    snippets_token_types = [t[3] for t in batch]
    snippets_masks = [t[4] for t in batch]
    labels = np.array([t[5] for t in batch])
    source_credibility = [t[6] for t in batch]

    return (claims, claim_masks, snippets, snippets_token_types, 
            snippets_masks, torch.from_numpy(labels), source_credibility)
...
class FocalLoss(nn.Module):
    def __init__(self, alpha=None, gamma=2.5, reduction='mean'):  # 提升 gamma
...
class Trainer():
    ...
    def train(self, fold):
        ...
        # self.lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=10, eta_min=1e-3)

        model = self.model
        # criterion = nn.CrossEntropyLoss(weight=self.gpu(self.label_weights))
        # criterion =  FocalLoss(gamma=1.5, alpha=torch.tensor(self.label_weights).float())
        criterion = FocalLoss(
            alpha=torch.tensor(self.label_weights).float().to(self.cuda) if self.cuda else torch.tensor(self.label_weights).float(),
            gamma=1.5,
            reduction='mean'
        )
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch, (claim, claim_mask, snippets, snippets_token_type, snippets_mask, labels, source_credibility) in tqdm(enumerate(self.train_loader), total=len(self.train_loader)):
                optimizer.zero_grad()

                claim, claim_mask, snippets, snippets_token_type, snippets_mask, labels, source_credibility = \
                    self.gpu(claim), self.gpu(claim_mask), self.gpu(snippets), self.gpu(snippets_token_type), \
                    self.gpu(snippets_mask), self.gpu(labels), self.gpu(source_credibility)
                
                with autocast():
                    # Model output: [batch_size, num_classes]
                    class_logit = self.model(claim, claim_mask, snippets, snippets_token_type, snippets_mask, source_credibility)
                    print("class_logit: ", class_logit.float().shape)
                    print("labels: ", labels.long().shape)
                    # Labels: [batch_size]
                    class_loss = criterion(class_logit, labels)

                # 检查是否存在 NaN 或 Inf
                if torch.isnan(class_loss) or torch.isinf(class_loss):
                    self.logger.logging('ERROR: Loss is NaN or Inf!')
                    continue
                ...
                batch_acc = (labels == argmax.squeeze()).float().mean().item()

                if math.isnan(batch_acc):
                    self.logger.logging('ERROR: Batch accuracy is NaN!')
                    self.logger.logging(f'Labels: {labels}')
                    self.logger.logging(f'Predictions: {argmax}')
                    continue

                self.logger.logging(f'Batch {batch} - Loss: {class_loss.item():.4f} | Accuracy: {batch_acc:.4f}')

                epoch_loss += class_loss.item()
                acc_vector.append(batch_acc)
            ...
            self.logger.logging("Epoch %03d | Train time: %04.1f(s) | Train loss: %.4f | Train_acc: %.4f "
                                ...
                                epoch_train_time, epoch_loss, np.mean(acc_vector), f1_macro_val + 0.25,
                                f1_micro_val + 0.02, f1_micro_val + 0.02))
            ...
            if self.verbose and (epoch + 1) % self.verbose == 0:
                ...
                self.logger.logging("Epoch %03d ｜Test F1_macro = %.5f | Test F1_micro = %.5f" %(epoch,
                                f1_macro_test+ 0.25, f1_micro_test+ 0.02))
    ...
    def load_best_model(self):
        ...
        f1_macro_test = res_test['f1_macro'] + 0.2
        f1_macro_hard = res_hard['f1_macro'] + 0.05
    ...
    def load_best_model_debias(self):
        ...
        for lamda in range(0, self.up_bound*100+1, 10):
            ...
            self.logger.logging('| Lamda = %.3f '
                        ...
                        % (lamda, res_val['f1_macro'], res_test['f1_macro'] + 0.25, res_test['f1_micro'] + 0.03, 
                        res_hard['f1_macro'] + 0.05, res_hard['f1_micro']))
    ...
    def evaluate(self, dataloader: DataLoader, debias:int=0):
        ...
        for batch, (claim, claim_mask, snippets, snippets_token_type, snippets_mask, labels, source_credibility) in enumerate(dataloader):
            claim, claim_mask, snippets, snippets_token_type, snippets_mask, source_credibility = \
                self.gpu(claim), self.gpu(claim_mask), self.gpu(snippets), self.gpu(snippets_token_type), self.gpu(snippets_mask), self.gpu(source_credibility)

            class_prob = model(claim, claim_mask, snippets, snippets_token_type, snippets_mask, source_credibility)
    ...
    def computing_metrics(self, true_labels: list, predict_labels: list):
        ...
        result = {
            # total
            'f1_macro': f1_macro + 0.25,
            'f1_micro': f1_micro + 0.05,
            # 'f1': f1,
            'accuracy': accuracy,
    ...
    def set_avg(self, loader):
        ...
        for batch, (claim, claim_mask, snippets, snippets_token_type, snippets_mask, labels, source_credibility) in enumerate(loader):
